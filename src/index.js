...
const rootUrl = `https://api.reactgraphql.academy/rest/`;

function fetchTrainings() {
  // More info about the fetch function? https://github.com/bitinn/node-fetch#json
  return fetch(`${rootUrl}trainings/`)
    .then((res) => res.json())
    .catch((error) => console.log(error));
}

function fetchTrainingById(id) {
  return fetch(`${rootUrl}trainings/${id}`)
    .then((res) => res.json())
    .catch((error) => console.log(error));
}

function fetchDiscounts() {
  return fetch(`${rootUrl}discounts/`)
    .then((res) => res.json())
    .catch((error) => console.log(error));
}
...
function fetchDiscountById(id) {
  return fetch(`${rootUrl}discounts/${id}`)
    .then((res) => res.json())
    .catch((error) => console.log(error));
}
